...
| 高清放大 | UltimateSDUpscale (2x, 4x-UltraSharp), steps=6, denoise=0.3 | 放大后无明显伪影，细节清晰 |
| 局部重绘 | NSW修复（Flux2-Klein/SDXL）+ Klein高分重绘（3种模式） | 重绘区域光线/色调一致，边缘自然 |
...
支持三种重绘模式，共用 Canvas 遮罩绘制界面：

**Tab 1 - NSW修复（inpaint_flux / inpaint_sdxl）**
- Flux2-Klein 模式：F2K-9b-darkBeast_fp8，steps=6，cfg=1，euler，denoise=0.45，裁剪512×512
- SDXL-写实模式：XL-epicrealismXL，steps=15，cfg=3.5，dpmpp_2m_sde，karras，denoise=0.5，裁剪1024×1024
- WD14 自动识别prompt，用户可在输入框补充

**Tab 2 - Klein高分重绘（inpaint_klein）**
- Flux2-Klein ReferenceLatent 方案，denoise=1.0（全重绘）
- 用户直接输入编辑指令，裁剪1024×1024

**工作流节点共用逻辑**：LoadImage(原图+遮罩) → INPAINT_ExpandMask → InpaintCropImproved → WD14Tagger + CLIPTextEncode → InpaintModelConditioning → KSampler → VAEDecode → ColorMatch → InpaintStitchImproved → SaveImage
...
| V0.9 | Phase 1新增高清放大（UltimateSDUpscale）和局部重绘（3种模式：Flux2-Klein/SDXL/Klein高分重绘）；任务阶段扩展为generating/faceswapping/upscaling/done；P0-E验证计划（ZIB模型/解剖LoRA/放大方案）；工作流文件扩展至9个JSON |
